...
import type { User, Team, Driver, GrandPrix, PitStop, Strategy } from '../types';

interface EditStrategyProps {
  user: User | null;
}

export default function EditStrategy({ user }: EditStrategyProps) {
  ...
  const loadData = async (strategyId: number) => {
    try {
      // Load dropdown data
      ...
      // Load existing strategy (user endpoint only)
      const strategy = await strategyService.getByIdUser(strategyId);

      // Check permissions - only owner can edit
      if (strategy.user_id !== user!.id) {
        setError('You do not have permission to edit this strategy.');
        return;
      }
      ...
      setGrandPrixId(strategy.grand_prix_id);
      setDriverId(strategy.driver_id);
      setTeamId(strategy.team_id);
      setPitStops(strategy.parameters.pit_stops || []);
      setFuelLoad(strategy.parameters.fuel_load?.toString() || '');
      setFuelMode(strategy.parameters.fuel_mode || '');
      setTireStrategy(strategy.parameters.tire_strategy || '');
    } catch (err: any) {
      setError(err.response?.data?.detail || 'Failed to load strategy. You may not have permission to edit this strategy.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!grandPrixId || !driverId || !teamId || !id) {
    ...
    try {
      ...
      await strategyService.update(parseInt(id), strategyData);
      alert('Strategy updated successfully!');
      navigate(`/strategies/${id}`);
    } catch (err: any) {
      setError(err.response?.data?.detail || 'Failed to update strategy.');
